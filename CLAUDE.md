...
- **`kit/spec.yaml` must not `extends: claude`.** The built-in kit's `entrypoint` carries `--dangerously-skip-permissions` and its install command seeds `~/.claude/settings.json` with `"defaultMode": "bypassPermissions"` (both in the spec embedded in `sbx.exe`); inheritance is additive, a child `sandbox.entrypoint` is ignored, `sandbox.command: []` reads as unset, and the image's `ENTRYPOINT`/`CMD` never run — all verified. So the kit is standalone and carries copies of the parent's network allows, credential block, `IS_SANDBOX` and MCP-gateway registration (not its `~/.claude/*` volumes — they only matter for `sbx kit add`/`--model`); when sbx changes those, nothing propagates here by itself. `init-config.py` seeds the onboarding/trust flags in `~/.claude.json` that the parent's install step used to.
...
- **The `mcp` kit argument only hides the gateway from the agent.** `${{ kit.args.mcp }}` is substituted textually before the YAML is parsed, so the install step can gate itself on it (`--kit-arg mcp=on` at create time, per sandbox; the default is `off`). What it cannot do is remove the gateway: sbx provisions one for every sandbox and injects `MCP_GATEWAY_URL` with no flag or setting to opt out (`--static-mcp` needs a registered server and only suppresses `mcp-find`/`mcp-add`/`mcp-config-set`), and `mcp-gateway.docker.internal` is routed by the proxy outside policy evaluation, so a network `deny` does not touch it — verified by curl from inside with the deny in place.
